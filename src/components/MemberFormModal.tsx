...
import {
  X, User, CreditCard, Coins, DollarSign, Loader2, Globe, Calendar,
  MessageSquare, Info,
} from 'lucide-react';
...
export default function MemberFormModal({
  ...
  const initialChar = (form.first_name?.trim()?.[0] || form.last_name?.trim()?.[0] || '?').toUpperCase();
  const isEdit = disablePayment; // edit mode hides inline payment fields and shows payments section
      ...
      <div className="modal modal-lg">
        <div className="modal-header">
          <div className="modal-header-title">
            <div className="modal-header-icon" style={{ background: 'var(--purple-light)', color: 'var(--purple)' }}>
              {isEdit ? (
                <span style={{ fontSize: 14, fontWeight: 800 }}>{initialChar}</span>
              ) : (
        ...
        <form onSubmit={onSubmit}>
          <div className="modal-body">
            ...
            {/* Initial payment — Add mode only */}
            {!isEdit && (
              <div className="form-section">
                <div className="form-section-title">
                  <span className="section-icon"><DollarSign size={12} /></span>
                  {t('member.sectionPayment')}
                </div>
                <div className="form-row">
                  <div className="form-group">
            ...
            <div className="form-section">
              <div className="form-section-title">
                ...
                {t('member.sectionNotes')}
              </div>
              <div className="form-group">
                <label>{t('member.comment')}</label>
                <div className="input-with-icon">
                  <span className="input-icon" style={{ top: 14, transform: 'none' }}><MessageSquare size={14} /></span>
                  <textarea
            ...
            {isEdit && paymentsSection && (
              <div className="form-section">
                <div className="form-hint-banner" style={{ marginBottom: 12 }}>
                  <Info size={14} />
                  <span>{t('member.editPaymentHint')}</span>
                </div>
                {paymentsSection}
              </div>
            )}
